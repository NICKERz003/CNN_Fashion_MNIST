...
# --- Preprocess สำหรับ display / VGG16 (grayscale → 3 channels) ---
def preprocess_test_images(images):
    # Normalize
    images = images.astype('float32') / 255.0
    # ขยายช่องทางจาก 1 → 3
    if images.ndim == 3:  # (num_samples, 28,28)
        images = np.expand_dims(images, axis=-1)
    images = np.repeat(images, 3, axis=-1)  # 3 channels
    # Resize เป็น 32x32
    images_resized = np.array([np.array(Image.fromarray((img*255).astype(np.uint8)).resize((32,32))) for img in images])
    images_resized = images_resized.astype('float32') / 255.0
    return images_resized

x_test_display = preprocess_test_images(x_test)

# --- โหลดโมเดล ---
model = load_model("models/fashion_mnist_vgg16_transfer_30E.h5", compile=False)
# Build model ด้วย input shape ที่ถูกต้อง
model.build(input_shape=(None, 32, 32, 3))
